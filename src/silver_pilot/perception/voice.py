"""
模块名称：voice
功能描述：语音感知模块，基于 DashScope SenseVoice API 实现语音转文字（ASR）和
         情感标签提取。SenseVoice 原生支持情感识别（<|HAPPY|>、<|SAD|> 等），
         无需额外模型即可获取用户情绪状态，供 Supervisor 进行情绪感知型路由。

情感标签映射：
    SenseVoice 输出的情感标签        → AgentState.user_emotion
    <|HAPPY|>                      → HAPPY
    <|SAD|>                        → SAD
    <|ANGRY|>                      → ANGRY
    <|NEUTRAL|>                    → NEUTRAL
...
技术栈：DashScope SenseVoice API ("qwen3-asr-flash")
"""
...
class VoiceProcessor:
    """
    基于 DashScope SenseVoice API 的语音感知处理器。

    **核心能力**:
    1. 语音转文字（ASR）：支持中/英/日/韩/粤等 50+ 语种
    2. 情感识别：从 SenseVoice 原生输出中解析情感标签
    3. 语种检测：自动识别输入语种
